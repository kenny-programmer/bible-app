...
/// API.Bible / USFM-style book ids (e.g. JHN.3).
enum BookUSFM {
    ...
    private static let aliases: [String: String] = ["Psalm": "Psalms"]

    static func bookNameToUsfm(_ bookName: String) -> String? {
        let trimmed = bookName.trimmingCharacters(in: .whitespacesAndNewlines)
        let resolved = aliases[trimmed] ?? trimmed
        return nameToUSFM[resolved]
    }

    static func chapterId(bookName: String, chapter: Int) -> String? {
        guard let usfm = bookNameToUsfm(bookName) else { return nil }
        return "\(usfm).\(chapter)"
    }
}
